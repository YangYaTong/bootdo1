			...
			.bootstrapTable(
					{
						...
						url : prefix + "/searchFulfillingAndUpdateContract", // 服务器数据的加载地址
					//	showRefresh : true,
					//	showToggle : true,
					//	showColumns : true,
						iconSize : 'outline',
										...
										
{
									...
									formatter : function(value, row, index) {
										...
										var f = '<a class="btn btn-success btn-sm" href="#" title="上报审批"  mce_href="#" onclick="up(\''
												+ row.contractId
												+ '\')"><i class="fa fa-send"></i></a> ';
										var k = '<a class="btn btn-success btn-sm" href="#" title="变更"  mce_href="#" onclick="change(\''
											+ row.contractId
											+ '\')"><i class="fa fa-edit"></i></a> ';
										...
										if(row.state=="合同履行中"){
											return k;
										}else if(row.state=="变更待上报"){
											return f ;
										}else if(row.state=="变更审批未通过"){
											return d ;
										}
...
function up(id) {
	layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '800px', '520px' ],
		content : prefix + '/updateUp/' + id // iframe的url
	});
